...
- **The orchestrator orchestrates and nothing else.** Its context is the run's lifespan, so no instruction anywhere may have it write code, read source, or load the PRD/PRPs into its own context. Briefs pass paths, never contents; results come back as the structured return contract, never as diffs or command output (`process/agent-routing.md` §The orchestrator does no work of its own). Any new step that reads a large file must be a delegation instead.

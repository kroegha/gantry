...
4. **Agent routing (§4)**: the four contract agents (`code-reviewer`, `reality-checker`, `uat-test-agent`, `requirements-auditor`) are fixed — carry them into the table as-is and never substitute a discovered equivalent (`${CLAUDE_PLUGIN_ROOT}/process/agent-routing.md`). For the **capability** roles only, resolve each to a real `name:` from the live agent roster now — never copy an example table blind. Roles with no match are marked "unaided"; that is a valid plan, not a blocker.

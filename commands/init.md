...
3. **Inventory agents**: read `~/.claude/AGENT-INDEX.md` if present (fallback: list `~/.claude/agents/` + frontmatter). **No library is a normal, supported state** — record it in one line and plan to route capability roles to `general-purpose` with a role brief (`${CLAUDE_PLUGIN_ROOT}/process/agent-routing.md` §Resolution order). Never present it to the owner as something to fix.

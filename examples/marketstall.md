...
The owner arrives with an idea and a two-page brief. No PRD.
...
**G0** — the owner accepts this as the input document. Not as correct: three claims in it are already suspect and flagged as gaps.
...
**G1** — PRD v2.0 becomes the spec of record.
...
**G2** — sign-off authorises the autonomous run.
...
**Stopped once, as declared:** the live test-mode payment. The owner ran one transaction, confirmed the ledger row and the receipt, and approved.

**UAT** found what green tests did not: on a slow connection the AI generator could be double-submitted, charging two credits for one description. The read-only Test Agent reported it with reproduction steps; the orchestrator wrote a failing test, fixed it, and the Test Agent re-verified in the next round.

**G3** — production approved on the evidence: UAT sign-off, requirements audit, readiness verdict, deployment doc.
...
- [ ] Kernel carries the money-integrity invariants and a backup job appears in P-BOOTSTRAP
